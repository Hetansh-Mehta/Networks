...
### 1.3.1. HTTP Overview
*   The client's browser has been configured to automatically ask for web server's default web page, or *home page*.
*   Client's initial request asks the server to send the home page. The web server's software has been configured to know that the default web page is contained in a file called `home.htm`. 
*   Client receives the file from server and displays the contents of the file in client's web browser window. <br>
![HTTP Overview](./Pictures/HTTPOverview.png)

### 1.3.2. HTTP Protocol Mechanisms
...
*   HTTP provides functionality to ask for the contents of web pages, specifically by giving the web browser the ability to request files from the server and giving the server a way to return the content of those files.
